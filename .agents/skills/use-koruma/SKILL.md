...
5. Attach validators with field-level `#[koruma(...)]` attributes. Use
   `TypeName::<_>...` for generic validators so builder completion
...
- Use `#[koruma(each(Validator::<_>...))]` for per-element validation of `Vec<T>`, slices, arrays, and optional variants of those.
