...
pub fn expand_validator(mut input: ItemStruct) -> Result<TokenStream2, syn::Error> {
    ...
    let (with_value_impl, with_value_ref_impl) = if has_generics {
        ...
        let output_builder_ty = quote! {
            #builder_name<#(#type_param_names,)* #module_name::#set_value_type<S>>
        };
        ...
        let with_value_ref_impl = match value_field_capture {
            ValueFieldCapture::Capture => quote! {
                impl<#(#type_param_names,)* S: #module_name::State> koruma::BuilderWithValueRef<#inner_type>
                    for #builder_name<#(#type_param_names,)* S>
                where
                    #(#where_predicates,)*
        ...
        (with_value_impl, with_value_ref_impl)
    } else {
        let output_builder_ty = quote! { #builder_name<#module_name::#set_value_type<S>> };
        ...
        let with_value_ref_impl = match value_field_capture {
            ValueFieldCapture::Capture => quote! {
                impl<S: #module_name::State> koruma::BuilderWithValueRef<#inner_type>
                    for #builder_name<S>
                where
                    S::#value_assoc_type: koruma::bon::IsUnset,
                    #inner_type: ::std::clone::Clone,
